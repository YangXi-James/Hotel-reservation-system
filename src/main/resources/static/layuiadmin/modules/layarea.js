layui.define(['layer', 'form', 'laytpl'], function (exports) {
    ...
    Class.prototype.events = function () {
      ...
      let destinctionFilter = 'destinction-' + layarea._id;
  
      let provinceEl = options.elem.find('.province-selector');
      let cityEl = options.elem.find('.city-selector');
      let destinctionEl = options.elem.find('.destinction-selector');
      ...
      if(destinctionEl.attr('lay-filter')){
        destinctionFilter = destinctionEl.attr('lay-filter');
      }
      provinceEl.attr('lay-filter', provinceFilter);
      cityEl.attr('lay-filter', cityFilter);
      destinctionEl.attr('lay-filter', destinctionFilter);
      ...
      if(destinctionEl.data('value')){
        options.data.destinction = destinctionEl.data('value');
        options.data.destinctionCode = getCode('destinction', options.data.destinction, options.data.cityCode.slice(0, 4));
      }
      provinceEl.attr('lay-filter', provinceFilter);
      cityEl.attr('lay-filter', cityFilter);
      destinctionEl.attr('lay-filter', destinctionFilter);
      ...
      form.on('select('+cityFilter+')', function(data){
        ...
        if(options.data.provinceCode){
            ...
            renderdestinction(options.data.cityCode);
        }
  
        options.change(options.data);
      });
      form.on('select('+destinctionFilter+')', function(data){
        options.data.destinction = data.value;
        if(options.data.cityCode){
            options.data.destinctionCode = getCode('destinction', data.value, options.data.cityCode.slice(0, 4));
        }
        options.change(options.data);
      });
      ...
      function renderCity(provinceCode){
        ...
        form.render('select');
        renderdestinction(currentCode);
      }
  
      function renderdestinction(cityCode){
        let tpl = '<option value="">--选择区--</option>';
        let destinctionList = getList('destinction', cityCode.slice(0, 4));
        let currentCode = '';
        let currentName = '';
        destinctionList.forEach(function(_item){
          // if (!currentCode){
          //   currentCode = _item.code;
          //   currentName = _item.name;
          // }
          if(_item.name === options.data.destinction){
            currentCode = _item.code;
            currentName = _item.name;
          }
          tpl += '<option value="'+_item.name+'">'+_item.name+'</option>';
        });
        options.data.destinction = currentName;
        destinctionEl.html(tpl);
        destinctionEl.val(options.data.destinction);
